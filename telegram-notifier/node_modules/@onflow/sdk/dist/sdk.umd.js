(function (global, factory) {
  ...
  function isOldIdentifierSyntax(cadence) {
    let legacyContractIdentifiers = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  ...
  function createResolveCadence(context) {
    return async function resolveCadence(ix) {
      ...
      utilInvariant.invariant(!isOldIdentifierSyntax(cadence, context.legacyContractIdentifiers) || !isNewIdentifierSyntax(cadence), "Both account identifier and contract identifier syntax not simultaneously supported.");
  ...
  const extensionBuffer = ext => {
    if (ext == null) return undefined;
    return rlp.Buffer.from(ext, "hex");
  };
  const rlpEncode = v => {
  ...
  const preparePayloadSignatures = tx => {
    ...
    return tx.payloadSigs?.map(sig => {
      ...
      const base = [sig.signerIndex, sig.keyId, signatureBuffer(sig.sig)];
  ...
  const createSignableVoucher = ix => {
    ...
    const buildInsideSigners = () => findInsideSigners(ix).map(id => {
      ...
      const ext = ix.accounts[id].extensionData;
      if (ext != null) base.extensionData = ext;
      return base;
    });
      ...
      try {
        ...
        const outsidePayload = encodeTransactionEnvelope({
          ...
          payloadSigs: insideSigners.map(id => {
            ...
            const acc = ix.accounts[id];
            const ext = acc.extensionData;
            if (ext != null) base.extensionData = ext;
            return base;
          })
        });
  ...
  const encodeMessageFromSignable = (signable, signerAddress) => {
    ...
    const message = {
      ...
      payloadSigs: signable.voucher.payloadSigs.map(ps => {
        ...
        if (ps.extensionData != null) {
          base.extensionData = ps.extensionData;
        }
        return base;
      })
    };
